...
func RunLogs(args []string, loadRun RunLoader) int {
...
func FilterRunEvents(events []cliruntime.RunEvent, kind string) []cliruntime.RunEvent {
	kind = strings.TrimSpace(kind)
	if kind == "" || kind == "all" {
		return events
	}
	filtered := make([]cliruntime.RunEvent, 0, len(events))
	for _, ev := range events {
		switch kind {
		case "tool":
