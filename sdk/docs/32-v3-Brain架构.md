...
>
> **⚠️ 代码对照勘误（以当前代码为准，2026-04-26 更新）：**
> - §2.2 能力映射表：实际有 7 个专精大脑（补充 desktop），多任务支持已完成（✅）
> - §7.1 `/v1/jobs` 端点：✅ **已实现**（POST/GET/DELETE，复用 runs 底层，mode=background 筛选）
> - §7.4 ContextEngine.Compress 签名：实际为 `(ctx, messages, budget int)` 多了 budget 参数
> - §7.6 BrainLearner：✅ **有 `Adapt()` 方法**（`sdk/kernel/learning.go:724`）。5 脑（quant/browser/code/fault/verifier）有真实领域自适应实现；data/central 的 `Adapt()` 为空实现（`return nil`）；desktop 完全未实现。LearningEngine 仍是 struct 非 interface，方法名与设计文档一致。
...
> - §13.5 Dashboard：✅ **实际 22+ 路由**（非 8 个），含 `/v1/jobs`、`/v1/workflows`、WebSocket `/v1/dashboard/ws`、SSE `/v1/dashboard/events` 等。前端 SPA 5 面板（Overview/Brains/Events/Executions/Learning）
...
| 多任务支持 | `WorkflowEngine` + `Orchestrator.ExecuteWorkflow` | ✅ 已实现（run/serve/chat 三模式接入） |
...
| B-2 | Scheduler | ✅ Orchestrator.ExecuteWorkflow | 任务派发引擎：DAG 拓扑排序、分层并行、brain 选择（通过 CapabilityMatcher）、依赖检查 |
...
| 编号 | 工作项 | 基础 | 交付物 |
|------|--------|------|--------|
| C-1 | Workflow Engine | ✅ 已完成 | DAG 编排：节点是 TaskExecution，边是 Flow Edge（materialized / streaming）。Orchestrator.ExecuteWorkflow 已接入 chat / run / serve 三种模式。同层节点并行执行，支持拓扑排序 + 依赖管理。 |
...
|------|----------|----------|----------|----------|----------|
| **L0 Brain 级** | 每个专精 brain | 自身执行效果、领域参数优化 | 执行结果反馈 | brain 本地存储 | ✅ 6/6 脑已实现（quant/browser/code/fault/verifier/data/desktop）。data `Adapt()` 已于 2026-04-26 实现（`brains/data/brain_learner.go`）；desktop 已于 2026-04-26 新建 `brains/desktop/cmd/learner.go` |
| **L1 协作级** | Central | 哪个 brain 更擅长什么任务 | delegate 结果评分 | Central 记忆 | ⚠️ `LearningEngine` 有 `RecordDelegateResult`/`RankBrains`，未自动接入 delegate 路径 |
| **L2 策略级** | Central | delegation 顺序、工具组合效果 | 多步任务执行链路 | 策略库 | ⚠️ `LearningEngine` 有 `RecordSequence`/`RecommendOrder`，未自动接入执行路径 |
| **L3 用户级** | Central | 用户偏好、工作模式、上下文习惯 | 用户交互历史 | 用户 profile | ⚠️ `LearningEngine` 有 `RecordUserFeedback`/`GetPreference`，未自动接入执行路径 |
...
|-------|----------|--------|----------|----------|
| **Quant** | 策略权重、品种评分、信号阈值 | EWMA 衰减 + 盈亏反馈 | WeightAdapter 权重表、SymbolScorer 评分表 | ✅ 已实现（`handleExecute` 后调用 `RecordOutcome`，`brain/learn` RPC 已接入） |
| **Data** | 数据源可靠性、采集频率、异常模式 | 延迟/错误率统计 + 异常频率分析 | 数据源评分表、异常 pattern 库 | ✅ `RecordOutcome`/`ExportMetrics`/`Adapt()` 全部实现（`brains/data/brain_learner.go`）。`brain/learn` RPC ✅（`brains/data/sidecar/handler.go`） |
| **Code** | 代码风格偏好、review 通过率 | 用户接受/拒绝反馈 + review 结果 | 风格模型、review 统计表 | ✅ 已实现（生成复杂度/重试策略建议） |
| **Browser** | 网站结构模式、提取规则有效性、反爬策略 | 提取成功率 + 页面结构变更检测 | 站点模型库、选择器有效性表 | ✅ 已实现（生成 timeout/等待策略建议） |
| **Verifier** | 验证规则有效性、误报率、漏报率 | 验证结果与实际结果对比 | 规则权重表、误报/漏报统计 | ✅ 已实现（生成验证深度/覆盖建议） |
| **Fault** | 故障模式识别、恢复策略有效性、告警准确率 | 故障处理结果反馈 + 告警验证 | 故障模式库、恢复策略评分表 | ✅ 已实现（生成故障强度/观察窗口建议） |
...
| 编号 | 工作项 | 优先级 | 依赖 |
|------|--------|--------|------|
| E-1 | SQLite 驱动实现（`sdk/persistence/driver_sqlite.go`） | P0 | 无 | ✅ 已完成（WAL + 事务 + 15+ 表完整 schema） |
| E-2 | RunStore/PlanStore 迁移到 SQLite | P0 | E-1 | ✅ 已完成 |
| E-3 | LearningEngine 持久化（Save/Load brain_scores） | P0 | E-1 | ✅ 已完成（`learning_profiles`/`task_scores`/`sequences` 等表） |
| E-4 | 5 脑 L0 BrainLearner 实现 | P0 | E-3 | ✅ 6/6 脑已实现（quant/browser/code/fault/verifier/data/desktop）。data `Adapt()` 已于 2026-04-26 实现（`brains/data/brain_learner.go`）；desktop 已于 2026-04-26 新建 `brains/desktop/cmd/learner.go` |
| E-5 | L1-L3 接入 delegate 执行路径 | P0 | E-3 | ⚠️ `LearningEngine` 有 L1-L3 方法，未自动接入 delegate 执行路径 |
| E-6 | Context Engine LLM 摘要路径 | P1 | E-1 | ✅ 已实现（2026-04-26）。`Compress` 在 `e.Summarizer != nil` 时调用 `summarizeMessages`，保留最新 2 条消息后由 LLM 摘要。chat/run/buildOrchestrator 三处均注入带 Summarizer 的 ContextEngine |
| E-7 | Context SharedMessages 持久化 | P1 | E-1 | ✅ 已完成（`shared_messages` 表 + `sqliteSharedMessageStore`） |
| E-8 | Streaming Edge 打通到 WorkflowEngine | P1 | 无 | ✅ 已完成（`WorkflowEngine` 已支持 streaming edge，`executeStreamingProducer`/`executeNormalNode` 双模式） |
| E-9 | AuditLog 持久化 | P1 | E-1 | ✅ 已完成（`audit_logs` 表 + `sqliteAuditLogger`） |
| E-10 | upgrade/rollback 命令实现 | P2 | 无 | ✅ 已完成（含备份、验证、失败自动回滚） |
| E-11 | Package 签名校验 | P2 | 无 | ✅ 已完成（Ed25519 签名/校验 + Zip Slip 防护） |
| E-12 | Dashboard WebSocket 推送 | P2 | 无 | ✅ 已完成（`gorilla/websocket`，前端优先 WS 回退 SSE） |
...
Sprint E-3（依赖 E-2/E-3）：⚠️ 部分完成
  [E-4]  5 脑 L0 BrainLearner — ✅ 5/6 脑已实现
  [E-5]  L1-L3 接入执行路径 — ⚠️ 方法已存在，未自动接入
  [E-6]  Context Engine LLM 摘要 — ❌ 待实现
  [E-7]  Context SharedMessages 持久化 — ✅ 已完成
```
...
### 16.2 StreamComplete 实现（端到端双向流式）

- `sdk/kernel/llm_proxy.go` 的 `llm.stream` 已从 "fallback to complete" 改为真正的 `provider.Stream()` 聚合实现。
- 当请求中包含 `stream_id` 时，Brain 在读取 SSE 事件的**同时**通过 `rpc.Notify(MethodLLMStreamDelta, ...)` 将每个 `llm.StreamEvent` 实时推送到 sidecar（fire-and-forget，零 RTT）。
- sidecar 在 `sdk/sidecar/kernel_provider.go` 的 `Stream()` 中注册 channel，由 `channelStreamReader` 消费实时事件，实现跨端真正的逐 token 流式体验。
- **HTTP SSE 统一出口**：Brain `handleContractExecute` 支持 `?stream=true` 模式，通过 `events.MemEventBus` 将 LLM 增量、工具事件、Turn 事件实时推送给 HTTP 客户端。
  - Sidecar `StreamConsumer` + `progressToolObserver` 通过 `MethodBrainProgress` 将事件回传 Brain
  - Brain `LLMProxy.handleStream` 直接将 LLM 事件 `Publish` 到 `eventBus`
  - Brain `Orchestrator` / `handleContractExecute` 的 `MethodBrainProgress` handler 将事件转发到 `eventBus`
  - HTTP 客户端通过 `GET /v1/executions/{id}/events` 或 `POST /v1/contracts/execute?stream=true` 订阅 SSE
- 统一事件类型命名空间：`llm.message_start`、`llm.content_delta`、`llm.thinking_delta`、`llm.tool_call_delta`、`llm.message_end`、`agent.tool_start`、`agent.tool_end`、`agent.turn`、`execution.started`、`execution.done`、`execution.error`、`execution.cancelled`
- 流结束后，Brain 仍返回聚合的 `llmCompleteResponse` 作为完整结果；旧 sidecar（不带 `stream_id`）保持原有聚合行为，完全向后兼容。
- 协议常量：`protocol.MethodLLMStreamDelta = "llm/stream/delta"`、`protocol.MethodBrainProgress = "brain/progress"`。
