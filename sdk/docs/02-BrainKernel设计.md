...
> **⚠️ 路径迁移说明（2026-04-24）：**
> - 包路径（`admin-go/app/mvp/internal/...`、`workflow/executor/kernel/`）和接口名（`BrainAgent`→`Agent`、`BrainKind`→`Kind`）为 v2 时期命名。v3 代码以 `sdk/` 目录为准：`sdk/agent/`、`sdk/kernel/`、`sdk/errors/`。
> - CLI 主二进制：本文中 `easymvp-exec` 对应 brain 仓库的 `brain`（`cmd/brain/main.go`）。
> - 注册方式：本文中 `easymvp-exec brain register` 和 `~/.easymvp-exec/brains.yaml` 对应 brain 仓库的 `brain brain install` 和 `~/.brain/config.json` + `brain.json` manifest。详见 [29-第三方专精大脑开发指南 §3.2](./29-第三方专精大脑开发指南.md#32-注册到-v3-的四种方式)。
...
| 6 | **不处理 HTTP/SSE** | Kernel 核心不处理 HTTP 协议。HTTP 层在 `cmd/brain` 的 serve 模式里。Kernel 通过 `events.EventBus` 发布实时执行事件（LLM 增量、工具调用等），由 HTTP 层订阅并 SSE 推送给客户端——Kernel 只生产事件，不感知 HTTP |
...
| **进度与流式** | `$/progress` notification 基础支持 | 已实现：`brain/progress`（Sidecar→Host 工具/LLM 进度）+ `llm/stream/delta`（Host→Sidecar LLM 增量）+ `events.MemEventBus` → HTTP SSE 统一出口 | MCP 的 progress 太薄，没有 `llm/stream/delta` 这样的原生 LLM 增量协议，也没有 EventBus→SSE 的桥接 |
...
| 15 | `brain/progress` | `{kind, execution_id, brain_kind, tool_name, args, detail, ok, message, sent_at}` | —（notification） | 所有大脑。Sidecar 在 Agent Loop 执行过程中实时上报工具调用、LLM 增量、Turn 状态等细粒度进度。Host 解析后转发到 EventBus，供 HTTP SSE 客户端订阅 |
| 16 | `llm/stream/delta` | `{stream_id, event}` | —（notification） | 所有大脑。Host Brain 在代理 LLM Provider 的 SSE 流时，将每个增量实时推送给 Sidecar。Sidecar 通过 `channelStreamReader` 消费。Host 同时将该事件 Publish 到 EventBus 供 SSE 输出 |

##### 12.5.4.4 可选扩展方法（v2.1+）

| # | 方法 | 用途 |
|---|------|------|
| 17 | `llm.requestDirectAccess` | `LLMAccessHybrid` 模式下申请临时直连窗口 |
| 18 | `workspace.read` / `workspace.write` | 显式的工作区读写操作（替代 tool.invoke 的路径操作，提供更强语义） |
| 19 | `brain.introspect` | 运行时检查大脑当前状态（message 栈长度、已用步数等），供调试 |
...
# 7a) 流式模式：Host 实时推送 LLM 增量到 sidecar（fire-and-forget notification）
→ {"jsonrpc":"2.0","id":null,"method":"llm/stream/delta","params":{
    "stream_id":"stream-42",
    "event":{"type":"content.delta","data":{"text":"Hello","kind":"text"}}
  }}

# 7b) 流式模式：sidecar 实时上报工具执行进度到 Host（fire-and-forget notification）
← {"jsonrpc":"2.0","id":null,"method":"brain/progress","params":{
    "kind":"tool_start","execution_id":"exec-123","brain_kind":"code",
    "tool_name":"code.write_file","args":"{\"path\":\"main.go\"}","sent_at":1716789012345
  }}
...
- v1 协议冻结的方法：#1 ~ #16（必须全部实现）
- v1 可选方法：#17 ~ #19（主进程和 sidecar 协商支持）
...
1. **不提供对话式 UI**——Kernel 核心不暴露 HTTP/WebSocket/SSE 端点（这些在 `cmd/brain` 中）。Kernel 通过 `events.EventBus` 发布实时执行事件，由 HTTP 层订阅并 SSE 推送给客户端
