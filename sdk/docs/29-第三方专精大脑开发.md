...
### 3.1 实例：`easymvp-brain` 作为第三方领域大脑

`easymvp-brain` 是第三方专精大脑的典型场景：它**不替代** brain-v3 的内置大脑，而是**消费**它们的结果，叠加 EasyMVP 领域规则（审核/编译/裁决/返工/验收）。
...
- **反向调用**：通过 `RichBrainHandler` 拿到 `KernelCaller`，调 `llm.complete` 做领域决策，调 `tool.invoke` 驱动基础大脑的工具
- **产物输出**：`ExecuteResult` 里的 `Artifacts / Verification / FaultSummary` 按 [44-Browser-Brain开发计划 §9.2](./44-Browser-Brain开发计划.md#92-brain-v3-侧已提供仍需扩展的接口) 的规范返回，供 EasyMVP runtime adapter 消费

注册方式与任何第三方 brain 一致（详见 §3.2）。

> **核心原则**：`easymvp-brain` 在协议层、进程模型、版本策略上没有任何特权，它就是一份遵循本文档规范的第三方实现。

---

## 3.2 注册到 v3 的四种方式

v3 支持 **4 种注册路径**，按优先级从高到低排列。第三方 brain 任选其一即可，不需要同时做多种。

### 3.2.1 方式一：Manifest 自动发现（推荐，零配置）

把第三方 brain 按标准目录结构放好，v3 启动时自动扫描：

```text
~/.brain/brains/
└── <kind>/
    ├── brain.json          ← manifest 文件（必须）
    └── <entrypoint>        ← sidecar 二进制或其他 runtime 入口
```

或者**项目内开发时**直接放在项目根目录下：
...
扫描顺序（先发现的优先）：
1. 项目内 `central/brain.json`（kind = "central"）
2. 项目内 `brains/<kind>/brain.json`
3. `~/.brain/brains/<kind>/brain.json`
...
```json
{
  "schema_version": 1,
  "kind": "easymvp",
  "name": "EasyMVP Domain Brain",
  "brain_version": "1.0.0",
  "capabilities": [],
  "runtime": {
    "type": "native",
    "entrypoint": "brain-easymvp-sidecar"
  },
  "policy": {
...
关键字段：
- `kind`：大脑标识，如 `easymvp`
- `runtime.entrypoint`：相对于 manifest 目录的二进制路径，或绝对路径
- `runtime.args` / `runtime.env`：传给 sidecar 进程的额外参数和环境变量
- `metadata.model`：该 brain 反向调用 `llm.complete` 时使用的模型（可选）
- `metadata.auto_start`：是否在 Orchestrator 创建时就启动（可选，默认 false）

### 3.2.2 方式二：`brain brain install` 命令安装

```bash
# 从本地目录安装（目录内需包含 brain.json）
brain brain install ./easymvp-brain/

# 或从打包文件安装
brain brain install ./easymvp-brain-1.0.0.brainpkg
```

安装流程：
1. 加载并验证 `brain.json`
2. 复制到 `~/.brain/brains/<kind>/`
3. 写入 `.active` 标记为激活

其他管理命令：

```bash
brain brain list                    # 查看已安装的大脑
brain brain init                    # 从模板生成新 brain 项目
brain brain pack                    # 打包当前目录为 .brainpkg
brain brain activate <kind>         # 激活
brain brain deactivate <kind>       # 停用
brain brain uninstall <kind>        # 卸载
brain brain upgrade <kind> <path>   # 升级
brain brain rollback <kind>         # 回滚到上一版本
```

> ⚠️ 早期文档中出现的 `brain register` 命令**当前未实现 CLI 路由**。实际对应的就是 `brain brain install`。

### 3.2.3 方式三：`~/.brain/config.json` 配置驱动

直接在用户配置文件的 `brains` 数组中声明：
...
```json
{
  "brains": [
    {
      "kind": "easymvp",
      "binary": "/opt/easymvp/bin/easymvp-brain",
      "args": ["--listen", ":7801"],
      "env": ["EASYMVP_MODE=production"],
...
字段与 `kernel.BrainRegistration` 一一对应：
- `kind`：大脑标识
- `binary`：显式二进制路径（非空时优先于 manifest 中的 `entrypoint`）
- `args` / `env`：额外参数和环境变量
- `model`：LLM 代理模型
- `auto_start`：是否提前启动

### 3.2.4 方式四：二进制发现（Fallback，无 manifest 也能工作）

什么都不配置，只要二进制命名符合规则，v3 就能找到：

1. 主程序同目录找 `brain-<kind>-sidecar`（**优先**）
2. 找不到再找 `brain-<kind>`
3. 再找不到去 `PATH` 搜索
4. Windows 下自动加 `.exe`
...
```text
brain.exe                         ← 主程序
brain-easymvp-sidecar.exe         ← 第三方 brain（优先匹配）
brain-easymvp.exe                 ← fallback 匹配
```

代码参考：`cmd/brain/helpers.go:55-66`（`sidecarBinaryNamesForOS`）。

### 3.2.5 合并优先级

当多种方式同时存在时，v3 的合并规则是：

**manifest 发现 > config.json 配置 > 二进制 fallback**

具体实现见 `cmd/brain/helpers.go` 的 `mergeManifestAndConfigBrains`：
- 相同 `kind` 以 manifest 为准
- config.json 中未被 manifest 覆盖的条目作为 fallback 补充
- 如果两者都没有，最后走二进制发现

### 3.2.6 推荐流程（给 easymvp-brain）

```bash
# 1. 编译出独立二进制
go build -o brain-easymvp-sidecar ./cmd/easymvp-brain/

# 2. 创建 manifest 目录
mkdir -p ~/.brain/brains/easymvp
cp brain-easymvp-sidecar ~/.brain/brains/easymvp/

# 3. 写 brain.json
cat > ~/.brain/brains/easymvp/brain.json << 'EOF'
{
  "schema_version": 1,
  "kind": "easymvp",
  "name": "EasyMVP Domain Brain",
...
# 5. 启动时自动注册
brain serve
# → 日志会输出 "manifest-discovery: easymvp ..."
```
...
- [`sdk/sidecar/sidecar.go`](../../sdk/sidecar/sidecar.go:22)
...
- [`sdk/sidecar/sidecar.go`](../../sdk/sidecar/sidecar.go:85)
...
如果你在 Go 里消费这条结果链，优先走 [`sdk/protocol.ToolCallResult`](../../sdk/protocol/tool_call.go:33) 的公共 helper：
...
- [`brains/code/cmd/main.go`](../../brains/code/cmd/main.go:86)
- [`brains/browser/cmd/main.go`](../../brains/browser/cmd/main.go:130)
- [`sdk/sidecar/tool_call.go`](../../sdk/sidecar/tool_call.go:1)
...
- [`sdk/sidecar/loop.go`](../../sdk/sidecar/loop.go:12)
...
- [`sdk/sidecar/loop.go`](../../sdk/sidecar/loop.go:76)
...
- [`brains/code/cmd/main.go`](../../brains/code/cmd/main.go:24)
- [`brains/browser/cmd/main.go`](../../brains/browser/cmd/main.go:31)
...
- [`sdk/protocol/tool_call.go`](../../sdk/protocol/tool_call.go:1)
- [`sdk/kernel/orchestrator.go`](../../sdk/kernel/orchestrator.go:239)
...
当前默认解析逻辑（`sidecarBinaryNamesForOS`）会按以下优先级查找：

1. `brain-<kind>-sidecar`（**优先**，v3 推荐的标准命名）
2. `brain-<kind>`（向后兼容的 fallback）
3. 主程序同目录 → `PATH`
4. Windows 下自动尝试 `.exe` 后缀

代码在：

- [`cmd/brain/helpers.go`](../../cmd/brain/helpers.go:55)

所以第三方 brain 最简单的发布方式是：

```text
brain
brain-easymvp-sidecar      ← 优先匹配
brain-easymvp              ← fallback 匹配
```

全部放在同一目录。详见 [§3.2.4 二进制发现](#324-方式四二进制发现fallback无-manifest-也能工作)。
...
- [`sdk/agent/agent.go`](../../sdk/agent/agent.go:8)
...
- [`sdk/kernel/orchestrator_test.go`](../../sdk/kernel/orchestrator_test.go:206)
- [`sdk/kernel/orchestrator_process_test.go`](../../sdk/kernel/orchestrator_process_test.go:17)
- [`sdk/sidecar/sidecar_test.go`](../../sdk/sidecar/sidecar_test.go:1)
